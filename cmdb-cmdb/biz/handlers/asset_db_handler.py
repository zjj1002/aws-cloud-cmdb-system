...
from libs.common import data_sort
...
class DBHandler(BaseHandler):
    def get(self, *args, **kwargs):
        ...
        page_size = int(self.get_argument('page', default="1", strip=True))
        limit = int(self.get_argument('limit', default='888', strip=True))
        db_list = []
        ...
        with DBContext('r') as session:
            ...
            if key == 'tag_name' and value:
                ...
                db_info = session.query(DB).outerjoin(DBTag, DB.id == DBTag.db_id).outerjoin(Tag, Tag.id ==
                                                                                             DBTag.tag_id).filter(
                    Tag.tag_name == value).order_by(DB.id).all()
                for msg in db_info:
                    tag_list = []
                ...
                db_list = data_sort(db_list, "db_code")
                db_list_re = db_list[(int(page_size) - 1) * limit:page_size * int(limit)]
                return self.write(dict(code=0, msg='获取成功', count=count, data=db_list_re))
            ...
            if key and key != 'tag_name' and not value:
                    ...
                    with DBContext('r') as session:
                        ...
                        db_info = session.query(DB).filter(DB.id.in_(set_db_id_list)).filter(
                            or_(DB.db_code.like('%{}%'.format(key)),
                                ...
                                DB.state.like('%{}%'.format(key)),
                                DB.idc.like('%{}%'.format(key)),
                                DB.db_env.like('%{}%'.format(key)))).order_by(DB.id).all()
                ...
                for msg in db_info:
                    ...
                    db_list.append(data_dict)
                db_list = data_sort(db_list, "db_code")
                db_list = db_list[(int(page_size) - 1) * limit:page_size * int(limit)]
            ...
            if limit == 888:
                ...
                if self.is_superuser:
                    if key and value:
                        ...
                        db_info = session.query(DB).filter_by(**{key: value}).order_by(DB.id).all
                    else:
                        count = session.query(DB).count()
                        db_info = session.query(DB).order_by(DB.id).all()
                else:
                    ...
                    if key and value:
                        ...
                        db_info = session.query(DB).filter(DB.id.in_(set_db_id_list)).filter_by(
                            **{key: value}).order_by(DB.id).all()
                    else:
                        # 获取主机详情

                        count = session.query(DB).filter(DB.id.in_(set_db_id_list)).count()
                        db_info = session.query(DB).filter(DB.id.in_(set_db_id_list)).all()
