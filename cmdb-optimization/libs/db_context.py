...
def get_key_db_engine():
    databases = settings.get('read_aksk_db', 0)
    dbuser = databases['DBUSER_KEY']
    dbpwd = databases['DBPWD_KEY']
    dbhost = databases['DBHOST_KEY']
    dbport = databases.get('DBPORT_KEY', 0)
    dbname = databases['DBNAME_KEY']
    return create_engine('mysql+pymysql://{user}:{pwd}@{host}:{port}/{dbname}?charset=utf8'
                         .format(user=dbuser, pwd=quote_plus(dbpwd), host=dbhost, port=dbport, dbname=dbname),
                         pool_pre_ping=True)

class KEYDBContext(object):
    def __init__(self):
        engine = get_key_db_engine()
        self.__engine = engine
