...
#获取实例的family，size，size转化到xlarge的因子数
def get_rds_instance_type_info(instance_type,multiaz):
    family, size = instance_type.split(".")[1:3]

    num = get_factor_num_with_xlarge(size)
    if multiaz:
        num = 2 * num
    return family, "small", num

#获取实例的family，size，size转化到xlarge的因子数
def get_relasticache_instance_type_info(instance_type):
    family, size = instance_type.split(".")[1:3]
    num = get_factor_num_with_xlarge(size)
    return family, "small", num
