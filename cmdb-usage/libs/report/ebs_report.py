...
class EbsReportData(ReportData):
    _NAME = "EBS"

    def __init__(self, ec2_bill=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._ec2_bill = ec2_bill
        self._report_data = self._ec2_bill.get_no_ec2_ebs_bill()
    ...
    def merge_total_cost(self):
        ...
        :return:
        """
        self.report_data["TotalCost"] = self.report_data.iloc[:, 3:].apply(lambda x: x.sum(), axis=1)
        self.insert_to_db()
    ...
    def get_dep_total_bill(self):
        ...
        sql = """
            select %(dep_tag_name)s,sum(TotalCost) as EbsCost
            from ec2_report
...
if __name__ == '__main__':
    ...
    r = EbsReportData(ec2_bill=ec2_bill)
